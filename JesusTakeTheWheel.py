...
,"pyautogui.moveRel(random.randint(0,500), random.randint(0,500), random.randint(0,10), pyautogui.easeInOutQuad)"  # move mouse, start slow, end fast
,"pyautogui.click(button=random.choice(['left','right']), clicks=random.choice([1,2,3]))" #random clicking of mouse
,"pyautogui.dragRel(random.randint(-200,200), random.randint(-200,200), random.randrange(0, 3), button=random.choice(['left','right']))" # click and drag
,"pyautogui.press( random.choice( keyboard_keys ) )" # press random key
,"pyautogui.press( random.sample( buttonMashing_keys, random.randint(4, 50) ) )" # press random keys
,"pyautogui.hotkey('ctrl', 'n')"  # ctrl-c to new window
# ,"pyautogui.hotkey('ctrl', 'c')"  # ctrl-c to copy #causes keyinterrupt exception
...
while True:
	# print(random.sample( buttonMashing_keys, random.randint(4, 50) ));
	# sys.exit();
	# time.sleep( random.randint( 0, 5) );
